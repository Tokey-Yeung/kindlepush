...
def get_exif(file_path):  # 获取照片的拍摄信息
    ...
    if LensModel==None:
        try:
            img=pyexiv2.Image(file_path,encoding='gbk')
            LensModel=img.read_xmp()['Xmp.exifEX.LensModel']
            img.close()
        except:
...
def image_borde(exif_dict, file_path):  # 给图片加边框，并添加信息
    ...
    img_new.save("完成.jpg", quality=95, subsampling=0, dpi=(300.0, 300.0))


if __name__ == '__main__':
    file_path = r"D:\新建文件夹\DS7A5688.JPG"
    exif_dict = get_exif(file_path)
    #image_borde(exif_dict,file_path)
